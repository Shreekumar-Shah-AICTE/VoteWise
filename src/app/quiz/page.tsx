...
export default function QuizPage() {
  const shuffledQuestions = useMemo(
    () => shuffleArray(quizQuestions).slice(0, 15),
    []
  );
  ...
  const badge =
    pct >= 90
      ? { e: "🏆", t: "Election Expert", c: "text-yellow-400" }
      : pct >= 70
        ? { e: "🎓", t: "Informed Voter", c: "text-green-400" }
        : pct >= 50
          ? { e: "📚", t: "Active Learner", c: "text-blue-400" }
      ...
      <header className="border-b border-white/5 bg-gray-950/80 backdrop-blur-lg sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-lg font-semibold">
              <span aria-hidden="true">🧠</span> Election Quiz
            </h1>
          </div>
        ...
        <div className="h-1 bg-white/5">
          <div
            ...
            style={{
              width: `${((idx + 1) / shuffledQuestions.length) * 100}%`,
            }}
            role="progressbar"
            aria-valuenow={idx + 1}
            aria-valuemin={1}
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="glass-card p-6 animate-fade-in" key={idx}>
          <div className="flex items-center gap-2 mb-4">
            <span
              className={`text-xs px-2 py-1 rounded-full border ${getDifficultyColor(q.difficulty)}`}
            >
              {q.difficulty}
            </span>
          ...
          >
            {q.options.map((opt, i) => {
              ...
              if (answered) {
                ...
                cls +=
                  "bg-white/5 border-white/10 hover:bg-indigo-500/10 hover:border-indigo-500/30 text-gray-200 cursor-pointer focus-visible:ring-2 focus-visible:ring-indigo-400";
              }
              return (
                <button
                  key={i}
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="w-8 h-8 rounded-full border border-current flex items-center justify-center text-sm font-semibold flex-shrink-0">
          ...
          {answered && (
            <div
              className="p-4 rounded-xl bg-indigo-500/5 border border-indigo-500/20 mb-6 animate-fade-in"
              role="status"
              aria-live="polite"
            >
          ...
          {answered && (
            <button
              onClick={handleNext}
              className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl transition-all hover:scale-[1.02] focus-visible:ring-2 focus-visible:ring-indigo-400 animate-fade-in"
            >
              {idx + 1 >= shuffledQuestions.length
                ? "See Results"
                : "Next Question →"}
